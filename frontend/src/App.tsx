...
export default function App() {
    ...
    if (player_id != null && ws == null) {
        fetch(`http://localhost:9090/player-exists?id=${player_id}`)
            ...
            .then((exists) => {
                console.log(exists);
                if (exists) {
                    let ws = new WebSocket(`ws://localhost:9090/player-rejoin-game?id=${player_id}`);
    ...
    return (
        <Router>
            <PlayersContext.Provider value={{ players: players, setPlayers: setPlayers, ws: ws }}>
                <Routes>
                    <Route path='/lobby' element={<Lobby username={username} />} />
                    <Route path='/' element={<Home username={username} setWs={setWs} setUsername={setUsername} />} />
